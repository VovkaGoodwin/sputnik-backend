...
class RefreshTokenRepository extends TokenRepository {

  public function newToken(int $userId, string $accessTokenId = ''): RefreshToken {
    $token = $this->jwtFacade->issue(
      ...
      fn(Builder $builder, DateTimeImmutable $issuedAt) => $builder
        ...
        ->withClaim('accessTokenId', $accessTokenId)
    );

    return new RefreshToken($token);
  }

  public function parseToken(string $token): RefreshToken | null {
    try {
      ...
      return new RefreshToken($parsedToken);
